<article class="card card--1 m-0">
    ...
    <div class="card__img" style="background-image: url({{!$product->images()->get()->isEmpty() ? Storage::url($product->images()->first()->path) : 'https://picsum.photos/200'}})"></div>
    <a href="{{route('product.show', compact('product'))}}" class="card_link">
        <div class="card__img--hover" style="background-image: url({{!$product->images()->get()->isEmpty() ? Storage::url($product->images()->first()->path) : 'https://picsum.photos/200'}})"></div>
    </a>
